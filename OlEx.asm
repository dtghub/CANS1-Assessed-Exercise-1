...
;    if found /= nil then goto SearchListLoopDone
...
CmdDone
    load   R5,InputPtr[R0]     ; R5 := InputPtr 
    lea    R6,3[R0]            ; R6 : 3 (size of a command)
    add    R5,R5,R6            ; R5 := R5 + 3
    store  R5,InputPtr[R0]     ; InputPrt := R5 (InputPrt now points to next command)
    jump   CommandLoop[R0]     ; jump back to top of the command loop
CmdTerminate
...
; print " into list "     
    ...
    trap   R8,R9,R10           ; write " into list "
; print i
...
; print " from list "     
    ...
    lea    R10,11[R0]          ; R10 := string length
    trap   R8,R9,R10           ; write " from list "
; print i
    add    R1,R3,R0            ; R1 = i = index of list to be inserted into
    jal    R13,WriteValIntFixedWidth[R0]  ; write integer
    jal    R13,WriteNewLine[R0]
...
; print "Search list "
    lea    R8,2[R0]            ; R8 := trap code for write
    lea    R9,MsgSearch[R0]    ; R9 := &"Search list\n"
    lea    R10,12[R0]          ; R10 := string length
    trap   R8,R9,R10           ; write "Search list\n"
; print i
    add    R1,R3,R0            ; R1 = i = index of list to be inserted into
    jal    R13,WriteValIntFixedWidth[R0]  ; write integer
; print " for "
...
; print "Print list "
    ...
    lea    R10,11[R0]          ; R10 := string length
    trap   R8,R9,R10           ; write "Print list \n"
; print i
